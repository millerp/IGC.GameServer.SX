	...
	while (true)
	{
		...
		switch (buff[0])
		{
			...
			else
			{
				...
				bool bUseClientProtocolCore = true; // CliProtocolCore(buff, proto, size, enc); // DLL protocolcore
...
#define pGetClassExp			((DWORD(__cdecl*)()) 0x004FE3C4)
#define pGetExp					((double(__cdecl*)(DWORD, int, char, int)) 0x00AD368F)
...
void SendPacket(BYTE* buff, int len, int enc, int unk1)
{
	...
	if (buff[2] == 0xF1 && buff[3] == 0x01) // ban by hwid
	{
		...
		// Fix account id save for sXe3
		DWORD OldProtect;
		VirtualProtect((LPVOID)0x09DE83FC, 12, PAGE_EXECUTE_READWRITE, &OldProtect); // SXE3
		...
		memcpy((void*)0x09DE83FC, AccountID, 11); // SXE3
	...
	else if (buff[2] == 0x0E)
	{
		...
		lpMsg2->Agility = *(WORD*)((*(DWORD*)0x85F7168) + 0x11A) + *(WORD*)((*(DWORD*)0x85F7168) + 0x134); // SXE3
