...
void __declspec(naked) HookDCFunc()
{
	...
	{
		_asm{
			MOV DWORD PTR DS : [0x12DDFC4], 1; // SXE3
			mov EDX, 0x004E49FE; // SXE3
			jmp EDX;
...
void CGLiveThread() // not used, so not ported for sxe3
{
...
void ReconnectThread() // can u attach debugger? yes
{
	...
	while (g_ReconnectProcess != 6)
	{
		...
		if (g_ReconnectProcess == 1)
		{
			...
			MemSet(0x12A5390, 5, 1); // set to render ingame, SXE3
			//	g_ServerInfo->GetIp();
			do
			{
...
void RenderLoadingBar()
{
	...
	tRenderLoadingBar RenderLoadBar = (tRenderLoadingBar)0x715501; // SXE3
...
void __declspec(naked) HookExitCharSelectFunc()
{
	...
	g_UserQuit = 1;
	_asm {
		PUSH 0x010FA4D8; // SXE3
		//0043F249   68 68641E01      PUSH main.011E6468
		mov edx,0x43F397; // SXE3
		jmp edx;
	}
...
void __declspec(naked) HookExitFunc()
{
	...
	g_UserQuit = 1;
	_asm{
		PUSH 0x011054A0; // SXE3
		MOV EDX, 0x00729634; // SXE3
		JMP EDX;
	}
}