...
void CItemShop::LoadItemInfo(LPSTR pchFilename)
{
	...
	for (pugi::xml_node item = main.child("Item"); item; item = item.next_sibling())
	{
		...
		memset(ItemInfo.btItemExOption, -1, sizeof(ItemInfo.btItemExOption));

		for (int i = 0; i < exc_opt_list.size(); i++)
		{
			ItemInfo.btItemExOption[i] = exc_opt_list.at(i);
		}
...
void CItemShop::GCCashInventoryItemUse(LPOBJ lpObj, int Result, int UniqueCode, int AuthCode, int ID1, int ID2, int ID3)
{
	...
	else if(Iter->second.btItemType == 2)
	{
		...
		if (Iter->second.btItemExOption[0] == (BYTE)-2)
		{
			if (lpItemInfo->ItemKindA != ITEM_KIND_A_WING)
			{
				btExc = g_ItemOptionTypeMng.CommonExcOptionRand(lpItemInfo->ItemKindA, btNewExcOption);
			}

			else
			{
				btExc = g_ItemOptionTypeMng.WingExcOptionRand(lpItemInfo->ItemKindA, lpItemInfo->ItemKindB, btNewExcOption);
			}
		}

		else if (Iter->second.btItemExOption[0] == (BYTE)-3)
		{
			if (lpItemInfo->ItemKindA != ITEM_KIND_A_WING)
			{
				btExc = g_ItemOptionTypeMng.CommonExcOptionGetCount(lpItemInfo->ItemKindA, btNewExcOption, Iter->second.btItemExOption[1]);
			}

			else
			{
				btExc = g_ItemOptionTypeMng.WingExcOptionGetCount(lpItemInfo->ItemKindA, lpItemInfo->ItemKindB, btNewExcOption, Iter->second.btItemExOption[1]);
			}
		}
		...
		else
		{
			for (int k = 0; k < 9; k++)
			{
				if (Iter->second.btItemExOption[k] == -1)
				{
					continue;
				}

				if (Iter->second.btItemExOption[k] >= 6)
				{
					bool bOptionFound = false;

					for (int j = 0; j < 5; j++)
					{
						if (btNewExcOption[j] == Iter->second.btItemExOption[k])
						{
							bOptionFound = true;
							break;
						}
					...
					for (int j = 0; j < 5; j++)
					{
						...
						{
							btNewExcOption[j] = Iter->second.btItemExOption[k];
							break;
						}
					}
				}
				...
				{
					btExc |= (1 << (5 - Iter->second.btItemExOption[k]));
				}
			}
		}
		...
		ItemSerialCreateSend(lpObj->m_Index, 236, NULL, NULL, ITEMGET(Iter->second.wItemGroup, Iter->second.wItemType), Iter->second.btItemLevel, 
			...
			Iter->second.dwItemPeriodTime, btExc, Iter->second.btItemSetOption, periodtime, btNewExcOption, 0);
	}
	...
	else
	{
		...
		else
		{
			...
			if (Iter->second.btItemExOption[0] == (BYTE)-2)
			{
				btExc = g_ItemOptionTypeMng.CommonExcOptionRand(lpItemInfo->ItemKindA, SocketOption);
			}

			else if (Iter->second.btItemExOption[0] == (BYTE)-3)
			{
				btExc = g_ItemOptionTypeMng.CommonExcOptionGetCount(lpItemInfo->ItemKindA, SocketOption, Iter->second.btItemExOption[1]);
			}
				...
				for (int k = 0; k < 9; k++)
				{
					...
					if (Iter->second.btItemExOption[k] >= 6)
					{
						for (int j = 0; j < 5; j++)
						{
							...
							{
								SocketOption[j] = Iter->second.btItemExOption[k];
								break;
							}
						}
					}
			...
			if (g_SocketOptionSystem.IsEnableSocketItem(ITEMGET(Iter->second.wItemGroup, Iter->second.wItemType)) == TRUE)
			{
			...
			ItemSerialCreateSend(lpObj->m_Index, 235, NULL, NULL, ITEMGET(Iter->second.wItemGroup, Iter->second.wItemType), Iter->second.btItemLevel,
				Iter->second.btItemDurability, Iter->second.btItemSkill, Iter->second.btItemLuck, Iter->second.btItemOption,
				lpObj->m_Index, btExc, Iter->second.btItemSetOption, 0, SocketOption, MainAttribute);
		}
	}
