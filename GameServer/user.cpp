...
bool gObjSetCharacter(LPBYTE lpdata, int aIndex)
{
	...
	if (lpObj->MapNumber == MAP_INDEX_ACHERON || lpObj->MapNumber == MAP_INDEX_DEBENTER || lpObj->MapNumber == MAP_INDEX_URUK_MOUNTAIN)
	{
		short sX;
		short sY;
		short sLVL;
	...
	else
	{
		...
		if(lpObj->m_RecallMon >= 0 && (lpObj->Class < 678 || lpObj->Class > 680))
		{
			...
			if (lpObj->Class == 707) // Dark Wizard Phantom
			{
				...
				if (ObjectMaxRange(lpObj->m_RecallMon) == true && gObj[lpObj->m_RecallMon].Type == OBJ_USER)
				{
					memcpy(pCallMonsterViewportCreate.CharSet, gObj[lpObj->m_RecallMon].CharSet, 18);
				}
			}
							...
							if (lpObj->Type == OBJ_USER)
							{
								...
								if (lpTargetObj->m_RecallMon >= 0  && (lpTargetObj->Class < 678 || lpTargetObj->Class > 680))
								{
									...
									if (lpTargetObj->Class == 707) // Dark Wizard Phantom
									{
										...
										if (ObjectMaxRange(lpTargetObj->m_RecallMon) == true && gObj[lpTargetObj->m_RecallMon].Type == OBJ_USER)
										{
											memcpy(pCallMonsterViewportCreate.CharSet, gObj[lpTargetObj->m_RecallMon].CharSet, 18);
										}
									}
