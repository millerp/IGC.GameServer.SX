...
BYTE CGamblingItemBag::GetGamblingSystemItem(int aIndex, BYTE btMapNumber, BYTE cX, BYTE cY, BYTE btItemType)
{
	...
	if(GetBagCount() > 0)
	{
		if(m_iGamblingItemGetRate[btItemType] >= GetLargeRand() % 1000000)
		{
			...
			iExOption = g_ItemOptionTypeMng.CommonExcOptionGetCount(GetItemKindA(iType), btNewExcOption, iExOptionCnt);
		}
		else //else for common items like a bag
		{
			...
			if(BagObject[iDropItemNum].m_isexitem != 0)
			{
				...
				{
					if (GetItemKindA(iType) == ITEM_KIND_A_WING)
					{
						iExOption = g_ItemOptionTypeMng.WingExcOptionRand(GetItemKindA(iType), GetItemKindB(iType), btNewExcOption);
					}
					
					else
					{
						iExOption = g_ItemOptionTypeMng.CommonExcOptionRand(GetItemKindA(iType), btNewExcOption);
					}
