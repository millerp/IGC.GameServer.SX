...
bool CLuaBag::MakeItemFromBag(int aIndex, BAG_ITEM * m_Item, CItem & Item, time_t & DurationItem)
{
	...
	if (g_SocketOptionSystem.IsEnableSocketItem(Item.m_Type) == true)
	{
...
bool CLuaBag::MakeItemFromBagForGremoryCase(int aIndex, BAG_ITEM * m_Item, _stGremoryCaseItem & Item)
{
	...
	if (g_SocketOptionSystem.IsEnableSocketItem(Item.wItemID) == true)
	{
		for (int i = 0; i < SocketCount; i++)
		{
			Item.btItemSocketOption[i] = 0xFE;
		}

	}

	else if (g_CMuunSystem.IsStoneofEvolution(Item.wItemID) == true)
	{
		Item.btItemSocketOption[0] = MuunEvolutionItemID >> 8;
		Item.btItemSocketOption[1] = ITEM_GET_INDEX(MuunEvolutionItemID);
	}
			...
			for (int i = 0; i < 9; i++)
			{
				...
				if (iNewExcOption[i] >= 6)
				{
					...
					for (int j = 0; j < 5; j++)
					{
						...
						{
							Item.btItemSocketOption[j] = iNewExcOption[i];
							break;
						}
					}
				}
