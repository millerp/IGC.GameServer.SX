...
void LuckyItemManager::LuckyItemTicketExchange(LPOBJ lpObj)
{
	...
	if (pLuckyItemEquipment->iExOption)
	{
		...
		{
			ExOption = g_ItemOptionTypeMng.WingExcOptionRand(p->ItemKindA, p->ItemKindB, btNewExcOption);
		}

		else
		{
			ExOption = g_ItemOptionTypeMng.CommonExcOptionRand(p->ItemKindA, btNewExcOption);
		}
	}
	...
	ItemSerialCreateSend(lpObj->m_Index, 0xFF, 0, 0, Type, level, dur, Op1, Op2, Op3, lpObj->m_Index, ExOption, SetOption, 0, btNewExcOption, 0);
