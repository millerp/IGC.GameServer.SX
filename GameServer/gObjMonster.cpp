...
void gObjMonsterBaseAct(LPOBJ lpObj)
{
		...
		if ( lpObj->m_Attribute )
		{
			...
			else
			{
				...
				if (g_bSilvesterEntranceBlock == true && (GetTickCount64() - g_i64LordSilvesterBlockTime >= 60000))
				{
					...
					g_Log.Add("[LORDSILVESTER] Full HP Restored, Entrance unlocked");
				}
	...
	else if ( lpObj->m_ActState.Emotion == 1 )
	{
		if ( lpObj->Class == 673 )
		{
			...
			g_Log.Add("[LORDSILVESTER] Attacker found - Entrance locked");
		}
...
//		itemrate = 100;
	...
	if (item_drop)
	{
		...
		if ( ExtDropPer == TRUE )
		{
			...
			{
				NOption = g_ItemOptionTypeMng.WingExcOptionRand(p->ItemKindA, p->ItemKindB, btSocketOption);
			}

			else
			{
				NOption = g_ItemOptionTypeMng.CommonExcOptionRand(p->ItemKindA, btSocketOption);
			}
