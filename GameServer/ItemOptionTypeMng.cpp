			...
			if (this->m_CommonExtOptionType[j].OptionID != -1)
			{
					...
					this->m_CommonExtOptionType[j].ItemKindA_3 == lpAttr->ItemKindA)
				{
					...
					if (bOptionExist == true)
					{
						...
						this->_CalcEffectValue(lpObj, this->m_CommonExtOptionType[j].Operator, this->m_CommonExtOptionType[j].OptionValue,
							this->m_CommonExtOptionType[j].FormulaID, iEffectValue);

						if (iEffectValue > 0)
						{
							this->_SetOptionEffect(lpObj, this->m_CommonExtOptionType[j].OptionEffect, iEffectValue, i);
						}
					}
				}
			}
		...
		if (this->m_WingExtOptionType[i].OptionID != -1)
		{
				...
				this->m_WingExtOptionType[i].ItemKindB == lpAttr->ItemKindB)
			{
				...
				if (bOptionExist == true)
				{
					...
					this->_CalcEffectValue(lpObj, this->m_WingExtOptionType[i].Operator, this->m_WingExtOptionType[i].OptionValue,
						this->m_WingExtOptionType[i].FormulaID, iEffectValue);

					if (iEffectValue > 0)
					{
						this->_SetOptionEffect(lpObj, this->m_WingExtOptionType[i].OptionEffect, iEffectValue, 7);
					}
				}
			}
		}
...
void CItemOptionTypeMng::_CalcEffectValue(OBJECTSTRUCT *lpObj, int iOperator, int iBaseValue, int iFormulaID, int & iEffectValue)
{
	switch (iOperator)
	{
		...
		case EXC_OPERATOR_PERCENT:
			iEffectValue = iBaseValue * iEffectValue / 100;
			break;
		...
		case EXC_OPERATOR_DIVIDE_BY_LIFE:
			iEffectValue = (lpObj->MaxLife + lpObj->Life) / iBaseValue;
			break;
		case EXC_OPERATOR_DIVIDE_BY_MANA:
			iEffectValue = (lpObj->MaxMana + lpObj->Mana) / iBaseValue;
			break;
		...
		case EXC_OPERATOR_FORMULA:
			{
				...
				int iUserLv = lpObj->Level;

				if (g_MasterLevelSkillTreeSystem.IsMasterLevelUser(lpObj) == TRUE)
				{
					iUserLv += lpObj->m_PlayerData->MasterLevel;
				}

				iUserLv /= iBaseValue;

				iEffectValue = g_FormulaParse.UseFormula(FORMULA_TYPE_EXCELLENT, iFormulaID, "x", 1, (double)iUserLv);
			}
			break;
	}
}

void CItemOptionTypeMng::_SetOptionEffect(OBJECTSTRUCT * lpObj, BYTE * OptionEffect, int iEffectValue, BYTE btItemPos)
{
	this->_SetEffect_Etc(lpObj, OptionEffect[0], iEffectValue);
	this->_SetEffect_Rate(lpObj, OptionEffect[1], iEffectValue);
	this->_SetEffect_Attack(lpObj, OptionEffect[2], iEffectValue, btItemPos);
	this->_SetEffect_Defense(lpObj, OptionEffect[3], iEffectValue);
	this->_SetEffect_Life(lpObj, OptionEffect[4], iEffectValue);
	this->_SetEffect_Mana(lpObj, OptionEffect[5], iEffectValue);
	this->_SetEffect_Skill(lpObj, OptionEffect[6], iEffectValue);
}

void CItemOptionTypeMng::_SetEffect_Etc(OBJECTSTRUCT * lpObj, BYTE OptionEffectID, int iEffectValue)
{
...
void CItemOptionTypeMng::_SetEffect_Rate(OBJECTSTRUCT * lpObj, BYTE OptionEffectID, int iEffectValue)
{
	switch (OptionEffectID)
	{
	case 0:
...
void CItemOptionTypeMng::_SetEffect_Attack(OBJECTSTRUCT * lpObj, BYTE OptionEffectID, int iEffectValue, BYTE btItemPos)
{
	switch (OptionEffectID)
	{
	case 0:
...
void CItemOptionTypeMng::_SetEffect_Defense(OBJECTSTRUCT * lpObj, BYTE OptionEffectID, int iEffectValue)
{
	switch (OptionEffectID)
	{
	case 0:
...
void CItemOptionTypeMng::_SetEffect_Life(OBJECTSTRUCT * lpObj, BYTE OptionEffectID, int iEffectValue)
{
	switch (OptionEffectID)
	{
	case 0:
...
void CItemOptionTypeMng::_SetEffect_Mana(OBJECTSTRUCT * lpObj, BYTE OptionEffectID, int iEffectValue)
{
	switch (OptionEffectID)
	{
	case 0:
...
void CItemOptionTypeMng::_SetEffect_Skill(OBJECTSTRUCT * lpObj, BYTE OptionEffectID, int iEffectValue)
{
	switch (OptionEffectID)
	{
	case 0:
